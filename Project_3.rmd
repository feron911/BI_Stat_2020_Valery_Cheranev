---
title: "Mouse_protein_expression"
...
# Список используемых библиотек (для дальнейшей работы с отчётом: "car", "MASS", "ggplot2","markdown","GGally", "psych", "Hmisc", "dplyr", "lmtest", "ggpubr", "vegan", "factoextra".
# Функция для установки библиотек используемых в отчёте (при необходимости). Не требует аргументов.
setup_libs <- function() {
  libs <- c("car", "MASS", "ggplot2","markdown","GGally", "psych", "Hmisc", "dplyr", "lmtest", "ggpubr", "vegan", "factoextra")
...
## Линейная модель для ERBB4_N на основании данных о других белках
  Исключаем из датафрейма все столбцы с метаданными, кроме классов, т.к. они дублируют всю необходимую информацию. Преобразуем стоблец Mouse ID, так чтобы можно было группировать и заменяем NA на среднее по классам и мышам в столбцах, в которых NA повторяется менее 15 раз. Затем повторяем процедуру для NA с использованием среднего по классу мышей.
```{r}
...
mddf_prot <- mddf_prot %>%
  ...
  mutate_at(vars(-group_cols()), funs(ifelse(is.na(.), mean(., na.rm = T), .)))
```  
  Строим линейную модель на основании всех данных о белках c учетом взаимодействия классов мышей.
```{r}
erbb4_lm <- lm(ERBB4_N ~ class*(.+.), mddf_prot)
summary(erbb4_lm)
```
  Проводим оценку линейной модели
  Гомоскедастичность остатков соблюдается и остатки модели распределены нормально
```{r}
...
```
  Однако наблдается очень высокая автокорреляция отдельных предикторов
```{r}
...
```
  Первые 4 компоненты вносят максимальный вклад.
